...
void rk4_stepper(double* y_old, const int nODE, double xstart, double xmax, double hstep, double eps, int& nxstep) {
  ...
  while (x0<xmax) {
    ...
    TCanvas*bungeeAnim = new TCanvas("bungeeAnim","SHO Animation",500,500);

    // Make a dummy histogram to set up the coordinate system, the x and y axes
    TH1D*bungee = new TH1D("bungee","Bungee 1-D",20,-1.5,1.5);
    bungee->SetMinimum(-134);
    bungee->SetMaximum(0);
    gStyle->SetOptStat(0); //Don't need a stat box in the canvas
    // Create the marker we are going to animate
    TMarker* massMarker = new TMarker(0,0,20);
    massMarker->SetMarkerColor(kBlue);
    massMarker->SetMarkerSize(3); // make the marker big, clearly visible
    TLine* lineAnim = new TLine(0,0,0,0);
    lineAnim->SetLineWidth(2);
    TArrow* velAnim = new TArrow(0,0,0,0,0.06,">");
    velAnim->SetLineWidth(2);
    velAnim->SetLineColor(3);
    TArrow* accAnim = new TArrow(0,0,0,0,0.06,">");
    accAnim->SetLineWidth(2);
    accAnim->SetLineColor(2);
    ...
    for (size_t iFrame=0; iFrame<xVals.size(); iFrame+=15)
    {
        ...
        massMarker->DrawMarker(0,yVals[0][iFrame]);
        lineAnim->DrawLine(0,0,0,yVals[0][iFrame]);
        if (yVals[1][iFrame]<0)
        {
            velAnim->DrawArrow(0,yVals[0][iFrame],0,yVals[0][iFrame]+yVals[1][iFrame],0.06,">");
        }
        if (yVals[1][iFrame]>0)
        {
            velAnim->DrawArrow(0,yVals[0][iFrame]+yVals[1][iFrame],0,yVals[0][iFrame],0.06,"<");
        }
        if(acc[iFrame]<0)
        {
            accAnim->DrawArrow(0,yVals[0][iFrame],0,yVals[0][iFrame]+acc[iFrame],0.06,">");
        }
        if(acc[iFrame]>0)
        {
            accAnim->DrawArrow(0,yVals[0][iFrame]+acc[iFrame],0,yVals[0][iFrame],0.06,"<");
        }
        // Display the message indicating the time of this frame
        sprintf(timeMessage,"t=%2.1f",xVals[iFrame]);
        sprintf(velMessage,"velocity=%2.1f",yVals[1][iFrame]);
        sprintf(accMessage,"acceleration=%2.1f",acc[iFrame]);
        ltx->DrawLatex(-1.5,0.5,timeMessage);
        lyx->DrawLatex(-0.8,0.5,velMessage);
        lax->DrawLatex(0.4,0.5,accMessage);
        // Update the canvas with the new objects that were drawn.
        bungeeAnim->Modified();
        bungeeAnim->Update();
        // To save the frame into an animated gif, use SaveAs, make sure the filetype is gif, and
        // add a +1 to tell root that this is to be saved in the same gif as an animation, with a
        // time delay of 1 ms between frames.
        if (iFrame<=xVals.size()-15)
        {
            cout << "iFrame = " << iFrame << endl;
            bungeeAnim->SaveAs("animationBungee.gif+1");
        }
...
/*
    ...
    rootMarker1->Draw();
    TText *t1 = new TText(tymax+2,lowpoint-5,"ymax=-118.194, x/L=1.11061");
    t1->SetTextAlign(22);
    ...
    rootMarker2->Draw();
    TText *t2 = new TText(tamax+5,accmax+1,"amax=18.776, Elastic force=2029.59");
    t2->SetTextAlign(22);
